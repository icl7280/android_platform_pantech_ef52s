...
extern struct dbus_debug_desc __start___debug_dbus[];
extern struct dbus_debug_desc __stop___debug_dbus[];

void __dbus_toggle_debug(void)
{
	struct dbus_debug_desc *desc;

	for (desc = __start___debug_dbus; desc < __stop___debug_dbus; desc++)
		desc->flags = ~(desc->flags & DBUS_DEBUG_FLAG_PRINT);
}
//20121017 P12116_BT_SYSTEM dbus dynamic logging ----
