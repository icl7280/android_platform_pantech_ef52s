...
//20121017 P12116_BT_SYSTEM dbus dynamic logging ++++
void __dbus_toggle_debug(void);

struct dbus_debug_desc {
	const char *file;
#define DBUS_DEBUG_FLAG_DEFAULT (0)
#define DBUS_DEBUG_FLAG_PRINT   (1 << 0)
	unsigned int flags;
...
#  define _dbus_verbose(fmt,...) do { \
	static struct dbus_debug_desc __dbus_debug_desc \
	__attribute__((used, section("__debug_dbus"), aligned(8))) = { \
		.file = __FILE__, .flags = DBUS_DEBUG_FLAG_DEFAULT, \
	}; \ 
	if (__dbus_debug_desc.flags & DBUS_DEBUG_FLAG_PRINT) {\
		_dbus_verbose_real( __FILE__,__LINE__,__FUNCTION__,fmt, ## __VA_ARGS__); \
	} \
} while (0)
//20121017 P12116_BT_SYSTEM dbus dynamic logging ----
#else
