...
//20121023 P12116_BT_SYSTEM QCT_patch_keep_sdp_alive_while_authentication ++++
#include "adapter.h"
#include "device.h"
#include "log.h"
//20121023 P12116_BT_SYSTEM QCT_patch_keep_sdp_alive_while_authentication ----
...
//20121023 P12116_BT_SYSTEM QCT_patch_keep_sdp_alive_while_authentication ++++
	...
	adapter = manager_find_adapter(&cached->src);
	if (!adapter)
		goto close_session;

	ba2str(&cached->dst, peer_addr);
	dev = adapter_find_device(adapter, peer_addr);
	if (!dev)
		goto close_session;

	if (device_is_authenticating(dev)) {
		DBG("Keepng SDP session for some more time");
		return TRUE;
	}
 
close_session:
	DBG("SDP is closing now");
//20121023 P12116_BT_SYSTEM QCT_patch_keep_sdp_alive_while_authentication ----
